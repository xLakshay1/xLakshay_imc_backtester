...
class Trader:
    ...
    def update_volatility(self, mid: float):
        self.mid_history.append(mid)
        if len(self.mid_history) > 20:
            self.mid_history.pop(0)

        if len(self.mid_history) < 5:
            return 2.0  # default spread offset

        returns = [
            abs(self.mid_history[i] - self.mid_history[i - 1])
    ...
    def trade_tomatoes(self, state: TradingState):
        ...
        # OPT 5: volatility-adaptive spread
        adaptive_offset = self.update_volatility(mid)

        # Inventory-aware quoting
        inventory_skew = position / 20.0
        buy_offset = adaptive_offset + max(0, inventory_skew)
        sell_offset = adaptive_offset + max(0, -inventory_skew)
