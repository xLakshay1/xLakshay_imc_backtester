...
class Trader:
    def __init__(self):
        ...
        self.day_shift = 20.0
        self.synthetic_strike_gap = 4000.0
        ...
        self.depth_p25 = 70.0
        self.depth_p75 = 90.0
        self.traderData = "R3_VEV4000_VV_ANCHOR_MINUS4000_V1"
    ...
    def trade_option(self, state: TradingState, memory: Dict[str, Any]) -> None:
        ...
        if depth_total < self.depth_min or spread > self.spread_cap:
            logger.print(
                f"VEV4000 skip depth={depth_total:.0f} spread={spread:.1f} fair={fair:.2f} mid={option_mid:.2f}"
            )
            return
        ...
        take_threshold = max(8.0, 0.45 * spread / max(0.90, regime_scale))
        passive_offset = max(2.0, 0.40 * spread / max(0.90, regime_scale))
        quote_size = max(8, int(round(self.base_quote_size * regime_scale)))
        if abs(position) > 0.60 * self.limit:
            quote_size = max(6, quote_size // 2)
        ...
        if edge >= take_threshold:
            self.place_buy(
                ...
                f"VEV4000 TAKE BUY edge={edge:.2f} fair={fair:.2f} vv_anchor={vv_anchor:.2f}",
            )
        if edge <= -take_threshold:
            self.place_sell(
                state,
                ...
                f"VEV4000 TAKE SELL edge={edge:.2f} fair={fair:.2f} vv_anchor={vv_anchor:.2f}",
            )
        ...
        if edge > take_threshold:
            self.place_buy(
                ...
                buy_price,
                quote_size,
                f"VEV4000 MM BUY q={quote_size} fair={fair:.2f} vv_anchor={vv_anchor:.2f}",
            )
        if edge < -take_threshold:
            self.place_sell(
                state,
                sell_price,
                quote_size,
                f"VEV4000 MM SELL q={quote_size} fair={fair:.2f} vv_anchor={vv_anchor:.2f}",
            )

        logger.print(
            f"vv_mid={vv_mid:.2f} vv_anchor={vv_anchor:.2f} vev_mid={option_mid:.2f} fair={fair:.2f} "
