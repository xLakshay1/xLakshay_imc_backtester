...
class Trader:
    def __init__(self):
        ...
        self.conversions = 0
        self.traderData = "MAINBH_OSMIUM_RET5_PRESSURE_PEPPER_SWEEP_ENTRY_V1"
    ...
    def trade_pepper(self, state: TradingState):
        ...
        if size > 0:
            order_depth = state.order_depths[product]
            limit_price = max(order_depth.sell_orders.keys())
            self.pepper_buy_orders += size
            self.send_buy_order(
                product,
                limit_price,
                size,
                f"PEPPER SWEEP START BUY {size} @ {limit_price}",
            )
