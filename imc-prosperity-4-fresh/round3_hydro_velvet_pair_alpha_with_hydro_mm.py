...
class Trader:
    def __init__(self) -> None:
        ...
        self.hydro_mm_base_size = 12
        self.hydro_mm_inventory_skew = 0.70
        self.hydro_mm_active_signal_cap = 1.60

        self.orders: Dict[Symbol, list[Order]] = {}
        self.buy_reserved: Dict[Symbol, int] = {}
        self.sell_reserved: Dict[Symbol, int] = {}
        self.conversions = 0
        self.traderData = "R3_HYDRO_VELVET_PAIR_WITH_HYDRO_MM_V1"
    ...
    def trade_hydro_mm(
        ...
        if hydro_spread <= 0:
            return
        if abs(pair_signal) > self.hydro_mm_active_signal_cap:
            return

        hydro_pos = self.get_position(state, self.hydro)
        pos_ratio = hydro_pos / float(self.hydro_limit)
        quote_fair = hydro_fair - self.hydro_mm_inventory_skew * pos_ratio * hydro_spread

        passive_offset = max(1.0, 0.45 * hydro_spread)
        buy_price = min(hydro_bid + 1, int(math.floor(quote_fair - passive_offset)))
        sell_price = max(hydro_ask - 1, int(math.ceil(quote_fair + passive_offset)))
        if buy_price >= sell_price:
            buy_price = min(hydro_bid, sell_price - 1)

        size_scale = max(0.4, 1.0 - abs(pair_signal) / max(self.hydro_mm_active_signal_cap, 1e-9))
        quote_size = max(4, int(round(self.hydro_mm_base_size * size_scale)))
        if abs(hydro_pos) > 0.50 * self.hydro_limit:
        ...
        self.place_buy(
            ...
            buy_price,
            quote_size,
            f"{self.hydro} MM BUY {quote_size} @ {buy_price} fair={hydro_fair:.2f} pair={pair_signal:.3f}",
        )
        self.place_sell(
            state,
            self.hydro,
            sell_price,
            quote_size,
            f"{self.hydro} MM SELL {quote_size} @ {sell_price} fair={hydro_fair:.2f} pair={pair_signal:.3f}",
        )
    ...
    def run_pair_alpha(self, state: TradingState, memory: Dict[str, Any]) -> None:
        ...
        hydro_spread = hydro_signal["spread"]
        velvet_bid = velvet_signal["best_bid"]
        velvet_ask = velvet_signal["best_ask"]

        if None in (hydro_mid, velvet_mid, hydro_bid, hydro_ask, hydro_spread, velvet_bid, velvet_ask):
            return
        ...
        hydro_spread = float(hydro_spread)
        ...
        hydro_fair = hydro_anchor + self.hydro_scale * velvet_z
        ...
        self.trade_hydro_mm(state, hydro_bid, hydro_ask, hydro_spread, hydro_fair, pair_signal)
        ...
        logger.print(
            ...
            f"hydro_fair={hydro_fair:.2f} hydro_spread={hydro_spread:.2f} "
