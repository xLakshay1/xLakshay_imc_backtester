...
class Trader:
    def __init__(self):
        ...
        self.depth_p25 = 70.0
        self.depth_p75 = 80.0
        self.protect_arm = 5000.0
        self.peak_floor_frac = 0.10
        self.resume_inventory = 16
        self.resume_band = 2.0

        self.orders: Dict[Symbol, list[Order]] = {}
        self.buy_reserved: Dict[Symbol, int] = {}
        self.sell_reserved: Dict[Symbol, int] = {}
        self.conversions = 0
        self.traderData = "R3_HYDROGEL_HYBRID_DEPTH_ANCHOR_PEAK_FLOOR10_V1"
    ...
    def parse_memory(self, trader_data: str) -> Dict[str, Any]:
        ...
        memory["cash"] = float(raw.get("cash", 0.0))
        memory["peak_pnl"] = float(raw.get("peak_pnl", 0.0))
        memory["floor_mode"] = bool(raw.get("floor_mode", False))
        seen_trade_keys = raw.get("seen_trade_keys", [])
        if isinstance(seen_trade_keys, list):
            memory["seen_trade_keys"] = [str(x) for x in seen_trade_keys][-80:]
        return memory
    ...
    def update_cash_from_own_trades(self, state: TradingState, memory: Dict[str, Any]) -> None:
        seen = set(memory.get("seen_trade_keys", []))
        updated = list(memory.get("seen_trade_keys", []))
        cash = float(memory["cash"])
        for trade in state.own_trades.get(self.product, []):
            key = (
                f"{trade.timestamp}|{trade.price}|{trade.quantity}|"
            ...
            if trade.seller == "SUBMISSION":
                cash += price * qty
        memory["cash"] = cash
        memory["seen_trade_keys"] = updated[-80:]
    ...
    def trade_hydrogel(self, state: TradingState, memory: Dict[str, Any]) -> None:
        ...
        self.update_cash_from_own_trades(state, memory)
        ...
        mtm_pnl = float(memory["cash"]) + position * mid
        peak_pnl = max(float(memory["peak_pnl"]), mtm_pnl)
        memory["peak_pnl"] = peak_pnl
        pnl_floor = peak_pnl * self.peak_floor_frac if peak_pnl >= self.protect_arm else -float("inf")
        if mtm_pnl <= pnl_floor:
            memory["floor_mode"] = True
        ...
        near_fair = abs(mid - fair) <= max(self.resume_band, 0.60 * float(signal["spread"]))
        if memory["floor_mode"]:
            if position > 0:
                flatten_size = max(8, min(position, take_size + quote_size))
                self.place_sell(
                    state,
                    best_bid,
                    flatten_size,
                    f"HYDRO FLOOR FLATTEN SELL @ {best_bid} pnl={mtm_pnl:.0f} peak={peak_pnl:.0f} floor={pnl_floor:.0f}",
                )
            elif position < 0:
                flatten_size = max(8, min(-position, take_size + quote_size))
                self.place_buy(
                    ...
                    best_ask,
                    flatten_size,
                    f"HYDRO FLOOR FLATTEN BUY @ {best_ask} pnl={mtm_pnl:.0f} peak={peak_pnl:.0f} floor={pnl_floor:.0f}",
                )

            if abs(position) <= self.resume_inventory and near_fair:
                memory["floor_mode"] = False
                memory["peak_pnl"] = max(mtm_pnl, self.protect_arm * self.peak_floor_frac)

            logger.print(
                f"mode=floor mid={mid:.2f} fair={fair:.2f} anchor={anchor:.2f} "
                f"ema={ema:.2f} pnl={mtm_pnl:.0f} peak={peak_pnl:.0f} floor={pnl_floor:.0f} pos={position}"
            )
            return
        ...
        logger.print(
            ...
            f"depth={depth_total:.0f} pos={position} pnl={mtm_pnl:.0f} peak={peak_pnl:.0f}"
        )
