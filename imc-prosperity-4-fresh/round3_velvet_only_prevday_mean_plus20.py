...
class Trader:
    def __init__(self):
        ...
        self.conversions = 0
        self.day_shift = 20.0
        self.traderData = "R3_VELVET_ONLY_PREVDAY_MEAN_PLUS20_V1"
    ...
    def update_day_anchor(self, memory: Dict[str, Any], timestamp: int, mid: float) -> None:
        ...
        if last_timestamp is not None and int(timestamp) < int(last_timestamp):
            if int(memory["day_count"]) > 0:
                memory["anchor"] = float(memory["day_sum"]) / float(memory["day_count"]) + self.day_shift
            memory["day_sum"] = 0.0
            memory["day_count"] = 0
