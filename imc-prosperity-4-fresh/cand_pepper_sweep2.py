...
class Trader:
    def __init__(self):
        ...
        self.conversions = 0
        self.traderData = "CAND_PEPPER_SWEEP2"
    ...
    def trade_pepper(self, state: TradingState):
        ...
        if size > 0:
            ...
            limit_price = sell_prices[min(1, len(sell_prices) - 1)]
            self.pepper_buy_orders += size
            self.send_buy_order(product, limit_price, size, f"PEPPER SWEEP2 START BUY {size} @ {limit_price}")
