...
class Trader:
    def __init__(self):
        ...
        self.conversions = 0
        self.traderData = "MAIN_SECURE2_STRICT_VOL_V1"
        ...
        self.osmium_abs_move_history: list[float] = []
        self.osmium_strict_vol_mode = False
        self.osmium_strict_vol_min_history = 180
        self.osmium_strict_vol_gap = 0.12
        self.osmium_strict_vol_floor_ratio = 1.08
    ...
    def trade_osmium(self, state: TradingState):
        ...
        strict_vol_mode, short_vol, medium_vol, long_vol = self.update_osmium_strict_vol_mode(abs_move)
        ...
        raw_vol_ratio = min(2.2, max(0.8, self.osmium_abs_move_ewma / self.osmium_base_vol))
        protection_ratio = raw_vol_ratio if strict_vol_mode else 1.0

        take_edge = 0.5 + 1.2 * protection_ratio if strict_vol_mode else 0.0
        ...
        inventory_skew = inv_ratio * (2.0 + (protection_ratio if strict_vol_mode else 0.0))

        base_offset = (2.0 if spread >= 4 else 3.0)
        if strict_vol_mode:
            base_offset += max(0.0, protection_ratio - 1.0)
        ...
        if abs(position) > 50:
            ...
            quote_size = 12
        if strict_vol_mode and protection_ratio > 1.45:
            quote_size = max(4, int(quote_size * 0.55))
        elif strict_vol_mode and protection_ratio > 1.2:
        ...
        logger.print(
            ...
            f"raw_vr={raw_vol_ratio:.2f} strict_vol={strict_vol_mode} "
            f"v40={short_vol:.2f} v60={medium_vol:.2f} v80={long_vol:.2f} pos={position} "
    ...
    def update_osmium_strict_vol_mode(self, abs_move) -> tuple[bool, float, float, float]:
        if abs_move is not None:
            self.osmium_abs_move_history.append(abs_move)
            if len(self.osmium_abs_move_history) > 360:
                self.osmium_abs_move_history = self.osmium_abs_move_history[-360:]

        if len(self.osmium_abs_move_history) < self.osmium_strict_vol_min_history:
            self.osmium_strict_vol_mode = False
            return False, 0.0, 0.0, 0.0

        short_vol = sum(self.osmium_abs_move_history[-40:]) / 40
        medium_vol = sum(self.osmium_abs_move_history[-100:-40]) / 60
        long_vol = sum(self.osmium_abs_move_history[-180:-100]) / 80

        strict_increase = (
            short_vol > medium_vol * (1 + self.osmium_strict_vol_gap)
            and medium_vol > long_vol * (1 + self.osmium_strict_vol_gap)
            and short_vol > self.osmium_base_vol * self.osmium_strict_vol_floor_ratio
        )
        self.osmium_strict_vol_mode = strict_increase
        return strict_increase, short_vol, medium_vol, long_vol
