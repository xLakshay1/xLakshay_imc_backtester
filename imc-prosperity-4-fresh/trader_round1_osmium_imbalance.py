...
class Trader:
    def __init__(self):
        ...
        self.conversions = 0
        self.traderData = "OSMIUM_TOP_IMBALANCE_PLUS_PEPPER_HOLD_V1"
        ...
        # Osmium state.
        # The data test showed total depth is not the spike trigger. The more
        # useful signal is top-of-book pressure: big best bid size combined
        # with weak best ask size.
        self.osmium_static_fair = 10000.0
        self.osmium_fair_value = None
        self.osmium_short_ema = None
        self.osmium_alpha = 0.025
        self.osmium_short_alpha = 0.18
        self.osmium_quote_size = 8
        self.osmium_take_edge = 6.0
        self.osmium_quote_edge = 4.0
        self.osmium_inventory_skew = 0.08
        self.osmium_microprice_weight = 0.18
        self.osmium_recent_mean_weight = 0.12
        self.osmium_imbalance_entry = 0.35
        self.osmium_imbalance_exit = 0.05
        self.osmium_min_bid_volume = 18
        self.osmium_max_ask_volume = 12
        self.osmium_max_spread_for_signal = 24
        self.osmium_signal_clip = 10
        self.osmium_signal_position_cap = 48
        self.osmium_pressure_fade_clip = 12
    ...
    def trade_osmium(self, state: TradingState):
        ...
        # Slow fair stays anchored near the product's fundamental center.
        ...
        if self.osmium_short_ema is None:
            self.osmium_short_ema = mid
        else:
            self.osmium_short_ema = (
                self.osmium_short_alpha * mid
                + (1 - self.osmium_short_alpha) * self.osmium_short_ema
            )
        ...
        top_volume = bid_volume + ask_volume
        if top_volume > 0:
            microprice = (
                best_ask * bid_volume + best_bid * ask_volume
            ) / top_volume
            top_imbalance = (bid_volume - ask_volume) / top_volume
        else:
            microprice = mid
            top_imbalance = 0.0
        ...
        pressure_signal = (
            top_imbalance >= self.osmium_imbalance_entry
            and bid_volume >= self.osmium_min_bid_volume
            and ask_volume <= self.osmium_max_ask_volume
            and spread <= self.osmium_max_spread_for_signal
            and microprice >= mid + 0.8
            and mid >= self.osmium_short_ema - 0.5
        )

        pressure_faded = (
            top_imbalance <= self.osmium_imbalance_exit
            or ask_volume >= bid_volume
            or mid < self.osmium_short_ema - 1.5
        )
        ...
        # Directional overlay: join the predicted up-burst only when the top
        # bid pressure is clear, and never let this leg fill the whole limit.
        if pressure_signal and position < self.osmium_signal_position_cap and max_buy > 0:
            room_to_cap = self.osmium_signal_position_cap - position
            size = min(max_buy, room_to_cap, self.osmium_signal_clip, max(1, ask_volume))
            if size > 0:
                self.osmium_buy_orders += size
                self.send_buy_order(
                    product,
                    ...
                    size,
                    msg=(
                        f"OSMIUM IMBALANCE BUY {size} @ {best_ask} "
                        f"imb={top_imbalance:.2f} bv={bid_volume} av={ask_volume}"
                    ),
                )

        # If we are long from pressure and the pressure disappears, flatten in
        # small chunks. This avoids holding the whole reversal after the burst.
        if position > 0 and pressure_faded and max_sell > 0:
            size = min(max_sell, position, self.osmium_pressure_fade_clip)
            if size > 0:
                self.osmium_sell_orders += size
                self.send_sell_order(
                    product,
                    ...
                    -size,
                    msg=(
                        f"OSMIUM IMBALANCE EXIT {size} @ {best_bid} "
                        f"imb={top_imbalance:.2f} bv={bid_volume} av={ask_volume}"
                    ),
                )

        # Still take very obvious mispricings around fair, but leave a wide
        # edge so the imbalance rule remains the main osmium signal.
        ...
        # controls. During strong pressure, do not fight the upward move by
        # leaning too aggressively on the ask.
        if spread >= 8:
            base_edge = 3
        elif spread >= 5:
            base_edge = 4
        ...
        if pressure_signal:
            sell_size = max(1, sell_size // 3)
            buy_size = min(buy_size + 3, 12)
        ...
        if pressure_signal:
            buy_price = min(best_ask, max(buy_price, best_bid + 1))
            sell_price = max(best_ask + 2, sell_price)
        ...
        logger.print(
            f"OSMIUM_IMB mid={mid:.2f} fv={fair_value:.2f} micro={microprice:.2f} "
            f"ema={self.osmium_short_ema:.2f} imb={top_imbalance:.2f} "
            f"bv={bid_volume} av={ask_volume} signal={pressure_signal} "
            f"fade={pressure_faded} pos={position} spread={spread} "
        ...
        if buy_size > 0:
            self.send_buy_order(
                ...
                buy_price,
                buy_size,
                msg=f"{product} IMB MM BUY {buy_size} @ {buy_price}",
            )
        if sell_size > 0:
            self.send_sell_order(
                product,
                sell_price,
                -sell_size,
                msg=f"{product} IMB MM SELL {sell_size} @ {sell_price}",
            )
