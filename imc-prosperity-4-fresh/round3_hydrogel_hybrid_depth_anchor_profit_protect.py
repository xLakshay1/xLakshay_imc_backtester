...
class Trader:
    def __init__(self):
        ...
        self.depth_p25 = 70.0
        self.depth_p75 = 80.0
        self.profit_protect_arm = 4000.0
        self.profit_drawdown_frac = 0.25
        self.resume_band = 3.0
        self.resume_inventory = 24

        self.orders: Dict[Symbol, list[Order]] = {}
        self.buy_reserved: Dict[Symbol, int] = {}
        self.sell_reserved: Dict[Symbol, int] = {}
        self.conversions = 0
        self.traderData = "R3_HYDROGEL_HYBRID_DEPTH_ANCHOR_PROFIT_PROTECT_V1"
    ...
    def parse_memory(self, trader_data: str) -> Dict[str, Any]:
        ...
        memory["protect_mode"] = bool(raw.get("protect_mode", False))
    ...
    def trade_hydrogel(self, state: TradingState, memory: Dict[str, Any]) -> None:
        ...
        drawdown_trigger = (
            peak_pnl >= self.profit_protect_arm
            and mtm_pnl <= peak_pnl * (1.0 - self.profit_drawdown_frac)
        )
        if drawdown_trigger:
            memory["protect_mode"] = True
        ...
        near_fair = abs(mid - fair) <= max(self.resume_band, 0.75 * float(signal["spread"]))
        if memory["protect_mode"]:
            if position > 0:
                ...
                self.place_sell(
                    ...
                    best_bid,
                    flatten_size,
                    f"HYDRO PROTECT FLATTEN SELL @ {best_bid} pnl={mtm_pnl:.0f} peak={peak_pnl:.0f}",
                )
            elif position < 0:
                flatten_size = max(8, min(-position, take_size + quote_size))
                self.place_buy(
                    ...
                    best_ask,
                    flatten_size,
                    f"HYDRO PROTECT FLATTEN BUY @ {best_ask} pnl={mtm_pnl:.0f} peak={peak_pnl:.0f}",
                )

            if abs(position) <= self.resume_inventory and near_fair:
                memory["protect_mode"] = False
                memory["peak_pnl"] = max(mtm_pnl, self.profit_protect_arm * 0.75)

            mode = "protect"
            logger.print(
                f"mode={mode} mid={mid:.2f} fair={fair:.2f} anchor={anchor:.2f} "
                f"ema={ema:.2f} pnl={mtm_pnl:.0f} peak={peak_pnl:.0f} pos={position}"
            )
            return
