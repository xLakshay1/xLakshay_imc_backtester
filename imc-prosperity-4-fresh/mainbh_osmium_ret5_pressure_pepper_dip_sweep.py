...
class Trader:
    def __init__(self):
        ...
        self.conversions = 0
        self.traderData = "MAINBH_OSMIUM_RET5_PRESSURE_PEPPER_DIP_SWEEP_V1"
        ...
        # Pepper: keep the buy-and-hold thesis, but wait briefly for the repeatable
        # opening dip seen across Round 1 and Round 2 before filling the long.
        self.pepper_limit = 80
        self.pepper_clip_size = 80
        self.pepper_initial_ask = None
        self.pepper_entry_started = False
        self.pepper_dip_entry_ticks = 3
        self.pepper_force_entry_time = 6_000
        self.pepper_entry_aggression = 12
    ...
    def trade_pepper(self, state: TradingState):
        ...
        position = self.get_product_pos(state, product)
        if self.pepper_initial_ask is None:
            self.pepper_initial_ask = best_ask

        dipped_enough = best_ask <= self.pepper_initial_ask - self.pepper_dip_entry_ticks
        force_entry = state.timestamp >= self.pepper_force_entry_time
        if dipped_enough or force_entry or position > 0:
            self.pepper_entry_started = True

        logger.print(
            f"PEPPER_DIP_ENTRY t={state.timestamp} bid={best_bid} ask={best_ask} "
            f"initial_ask={self.pepper_initial_ask} dipped={dipped_enough} "
            f"force={force_entry} started={self.pepper_entry_started} pos={position}"
        )

        if not self.pepper_entry_started:
            return
        ...
        if size > 0:
            ...
            self.send_buy_order(
                product,
                best_ask + self.pepper_entry_aggression,
                size,
                f"PEPPER DIP SWEEP BUY {size} @ {best_ask + self.pepper_entry_aggression}",
            )
