...
class Trader:
    def __init__(self):
        ...
        self.conversions = 0
        self.traderData = "MAINBH_OSMIUM_RET5_PRESSURE_PEPPER_MAKER_ENTRY_V1"
        ...
        # Pepper: buy most of the long immediately, then try to get the last slice
        # as maker/inside-spread flow before forcing the full buy-and-hold position.
        self.pepper_limit = 80
        self.pepper_clip_size = 80
        self.pepper_entry_core = 60
        self.pepper_maker_entry_end = 6_000
    ...
    def trade_pepper(self, state: TradingState):
        ...
        position = self.get_product_pos(state, product)
        logger.print(
            f"PEPPER_MAKER_ENTRY t={state.timestamp} bid={best_bid} ask={best_ask} pos={position}"
        )
        ...
        if position + self.pepper_buy_orders < self.pepper_entry_core:
            size = min(
                self.pepper_entry_core - position - self.pepper_buy_orders,
                remaining_buy,
            )
            if size > 0:
                self.pepper_buy_orders += size
                self.send_buy_order(
                    ...
                    best_ask,
                    size,
                    f"PEPPER CORE BUY {size} @ {best_ask}",
                )
            return

        if state.timestamp < self.pepper_maker_entry_end and best_ask - best_bid > 1:
            maker_price = best_ask - 1
            size = min(remaining_buy, self.pepper_limit - self.pepper_entry_core)
            if size > 0:
                self.pepper_buy_orders += size
                self.send_buy_order(
                    product,
                    maker_price,
                    size,
                    f"PEPPER MAKER ENTRY BUY {size} @ {maker_price}",
                )
            return
        ...
        if size > 0:
            ...
            self.send_buy_order(
                ...
                best_ask,
                size,
                f"PEPPER FORCE BUY {size} @ {best_ask}",
            )
