...
class Trader:
    def __init__(self):
        ...
        self.initial_velvet_mean = 5260.0
        self.velvet_fast_ema_alpha = 0.12
        self.velvet_ret_weight = 0.30
        self.velvet_total_imb_weight = 1.00
        self.velvet_l1_imb_weight = 0.85
        self.velvet_inventory_skew = 0.85
        self.velvet_base_quote_size = 32
        self.velvet_depth_p25 = 110.0
        self.velvet_depth_p75 = 132.0
        ...
        self.voucher_limit = {symbol: 300 for symbol in self.vouchers}
        ...
        self.voucher_inventory_skew = 0.006
        self.voucher_base_quote_size = 18
        self.voucher_depth_p25 = 55.0
        self.voucher_depth_p75 = 95.0
        ...
        self.conversions = 0
        self.traderData = "R3_VELVET_PLUS_OPTIONS_SMILE_HYBRID_V1"

    def parse_memory(self, trader_data: str) -> Dict[str, Any]:
        memory = {
            "velvet_anchor": self.initial_velvet_mean,
            "velvet_day_sum": 0.0,
            "velvet_day_count": 0,
            "velvet_ema": self.initial_velvet_mean,
            "velvet_prev_mid": None,
            "last_timestamp": None,
        }
        if not trader_data:
            return memory
        try:
            ...
            return memory
        memory["velvet_anchor"] = float(raw.get("velvet_anchor", self.initial_velvet_mean))
        memory["velvet_day_sum"] = float(raw.get("velvet_day_sum", 0.0))
        memory["velvet_day_count"] = int(raw.get("velvet_day_count", 0))
        memory["velvet_ema"] = float(raw.get("velvet_ema", memory["velvet_anchor"]))
        memory["velvet_prev_mid"] = raw.get("velvet_prev_mid")
    ...
    def limit_for(self, product: str) -> int:
        if product == self.velvet:
            return self.velvet_limit
        return self.voucher_limit[product]
    ...
    def update_velvet_anchor(self, memory: Dict[str, Any], timestamp: int, mid: float) -> None:
        ...
        if last_timestamp is not None and int(timestamp) < int(last_timestamp):
            ...
            memory["velvet_ema"] = float(memory["velvet_anchor"])
            memory["velvet_prev_mid"] = None
        memory["velvet_day_sum"] = float(memory["velvet_day_sum"]) + mid
    ...
    def trade_velvet(self, state: TradingState, memory: Dict[str, Any]) -> None:
        if self.velvet not in state.order_depths:
            return
        signal = self.book_signal(state.order_depths[self.velvet])
        mid = signal["mid"]
        ...
        self.update_velvet_anchor(memory, state.timestamp, mid)
        prev_mid = memory["velvet_prev_mid"]
        recent_move = 0.0 if prev_mid is None else mid - float(prev_mid)
        memory["velvet_prev_mid"] = mid

        previous_ema = float(memory["velvet_ema"])
        ema = self.velvet_fast_ema_alpha * mid + (1.0 - self.velvet_fast_ema_alpha) * previous_ema
        memory["velvet_ema"] = ema

        regime_scale = self.regime_scale(float(signal["depth_total"]), self.velvet_depth_p25, self.velvet_depth_p75)
        thick_book = float(signal["depth_total"]) >= self.velvet_depth_p75
        anchor = ema if thick_book else float(memory["velvet_anchor"])
        fair = anchor
        if thick_book:
            fair -= regime_scale * self.velvet_ret_weight * recent_move
        fair += regime_scale * float(signal["spread"]) * (
            -self.velvet_total_imb_weight * float(signal["imbalance"])
            + self.velvet_l1_imb_weight * float(signal["l1_imbalance"])
        )

        position = self.get_position(state, self.velvet)
        position_ratio = position / float(self.velvet_limit)
        quote_fair = fair - self.velvet_inventory_skew * position_ratio * float(signal["spread"])
        ...
        quote_size = max(8, int(round(self.velvet_base_quote_size * regime_scale)))
        if abs(position) > 0.60 * self.velvet_limit:
        ...
        logger.print(
            f"VELVET mode={'ema-thick' if thick_book else 'prevday-thin'} "
            f"mid={mid:.2f} anchor={anchor:.2f} ema={ema:.2f} "
            f"day_count={int(memory['velvet_day_count'])} ret={recent_move:.2f} "
            f"imb={float(signal['imbalance']):.3f} l1={float(signal['l1_imbalance']):.3f} pos={position}"
        )
    ...
    def trade_voucher(
        ...
        regime_scale = self.regime_scale(float(signal["depth_total"]), self.voucher_depth_p25, self.voucher_depth_p75)
        ...
        quote_size = max(6, int(round(self.voucher_base_quote_size * regime_scale)))
        if abs(position) > 0.60 * self.voucher_limit[product]:
            quote_size = max(4, quote_size // 2)
        ...
        quote_fair = fair_price - adjusted_edge - self.voucher_inventory_skew * position
    ...
    def trade_options(self, state: TradingState) -> None:
        if self.velvet not in state.order_depths:
            return
        underlying_mid = self.wall_mid(state.order_depths[self.velvet])
        ...
        for product in self.vouchers:
            if product not in state.order_depths:
                continue
            market_price = self.wall_mid(state.order_depths[product])
        ...
        for product in self.trade_vouchers:
            ...
            self.trade_voucher(state, product, fair_price, observed[product]["market_price"], signal)

    def run(self, state: TradingState):
        self.reset_orders(state)
        memory = self.parse_memory(state.traderData)
        self.trade_velvet(state, memory)
        self.trade_options(state)
