...
class Trader:
    def __init__(self):
        ...
        self.trade_vouchers = ["VEV_4000", "VEV_5400", "VEV_5500"]
        ...
        self.entry_threshold = 1.80
        self.take_threshold = 2.30
        self.inventory_skew = 0.006
        self.take_size = 28
        self.mm_size = 10
        self.depth_min = 70.0
        self.spread_max = 8.0
        self.book_weight = 0.45
        self.l1_weight = 0.25
        self.passive_offset_mult = 0.65
        self.traderData = "R3_OPTIONS_EXECUTABLE_V1"
    ...
    def trade_voucher(
        ...
        if spread > self.spread_max or depth_total < self.depth_min:
            logger.print(f"{product} SKIP spread={spread:.1f} depth={depth_total:.1f}")
            return
        ...
        if adjusted_edge < -self.take_threshold and best_ask <= fair_price - 0.50:
            self.place_buy(
                state,
                product,
                int(best_ask),
                min(int(signal["best_ask_volume"]), self.take_size),
                f"{product} TAKE BUY @ {best_ask} fair={fair_price:.2f} edge={adjusted_edge:.2f}",
            )
        elif adjusted_edge > self.take_threshold and best_bid >= fair_price + 0.50:
            self.place_sell(
                state,
                product,
                int(best_bid),
                min(int(signal["best_bid_volume"]), self.take_size),
                f"{product} TAKE SELL @ {best_bid} fair={fair_price:.2f} edge={adjusted_edge:.2f}",
            )
        elif abs(adjusted_edge) >= self.entry_threshold:
            ...
            if buy_price < sell_price:
                if adjusted_edge < 0:
                    self.place_buy(
                        ...
                        buy_price,
                        self.mm_size,
                        f"{product} PASSIVE BUY {self.mm_size} @ {buy_price} fair={fair_price:.2f} edge={adjusted_edge:.2f}",
                    )
                else:
                    self.place_sell(
                        state,
                        ...
                        sell_price,
                        self.mm_size,
                        f"{product} PASSIVE SELL {self.mm_size} @ {sell_price} fair={fair_price:.2f} edge={adjusted_edge:.2f}",
                    )
            else:
                logger.print(f"{product} SKIP crossed passive edge={adjusted_edge:.2f}")
        else:
            logger.print(
                f"{product} PASS fair={fair_price:.2f} mkt={market_price:.2f} "
                f"edge={adjusted_edge:.2f} spread={spread:.1f} depth={depth_total:.1f}"
            )
    ...
    def run(self, state: TradingState):
        ...
        a, b, c = fit
        logger.print(
            f"EXEC SMILE FIT a={a:.4f} b={b:.4f} c={c:.4f} spot={underlying_mid:.2f} tau_d={tau * 365.0:.2f}"
        )

        for product in self.trade_vouchers:
            if product not in observed or product not in state.order_depths:
                logger.print(f"{product} SKIP missing/unstable")
                continue
