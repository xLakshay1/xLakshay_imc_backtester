...
class Trader:
    def __init__(self):
        ...
        self.depth_p25 = 70.0
        self.depth_p75 = 80.0
        self.flatten_band = 3.0
        self.inventory_soft_limit = 0.30
        self.inventory_hard_limit = 0.60

        self.orders: Dict[Symbol, list[Order]] = {}
        self.buy_reserved: Dict[Symbol, int] = {}
        self.sell_reserved: Dict[Symbol, int] = {}
        self.conversions = 0
        self.traderData = "R3_HYDROGEL_HYBRID_DEPTH_ANCHOR_SAFER_V1"
    ...
    def trade_hydrogel(self, state: TradingState, memory: Dict[str, Any]) -> None:
        ...
        near_fair = abs(mid - fair) <= max(self.flatten_band, 0.60 * float(signal["spread"]))
        if abs(position) > self.inventory_hard_limit * self.limit:
            quote_size = max(4, quote_size // 2)
        elif abs(position) > self.inventory_soft_limit * self.limit:
            quote_size = max(4, int(round(0.65 * quote_size)))
        ...
        if near_fair and position > 0:
            flatten_size = min(
                position,
                max(8, int(round((14 if thick_book else 10) * regime_scale))),
            )
            self.place_sell(
                state,
                best_bid,
                flatten_size,
                f"HYDRO FLATTEN SELL @ {best_bid} fair={fair:.2f} anchor={anchor:.2f}",
            )
        elif near_fair and position < 0:
            flatten_size = min(
                -position,
                max(8, int(round((14 if thick_book else 10) * regime_scale))),
            )
            self.place_buy(
                ...
                best_ask,
                flatten_size,
                f"HYDRO FLATTEN BUY @ {best_ask} fair={fair:.2f} anchor={anchor:.2f}",
            )
        ...
        allow_two_sided_mm = thick_book and not near_fair
        if not thick_book:
            quote_size = max(4, quote_size // 2)

        if allow_two_sided_mm:
            self.place_buy(
                state,
                buy_price,
                quote_size,
            ...
            self.place_sell(
                ...
                sell_price,
                max(4, quote_size),
                f"HYDRO REDUCE MM SELL {quote_size} @ {sell_price} fair={fair:.2f} anchor={anchor:.2f}",
            )
        elif position < 0:
            self.place_buy(
                state,
                buy_price,
                max(4, quote_size),
                f"HYDRO REDUCE MM BUY {quote_size} @ {buy_price} fair={fair:.2f} anchor={anchor:.2f}",
            )
        ...
        logger.print(
            ...
            f"depth={depth_total:.0f} pos={position} near_fair={int(near_fair)} "
            f"two_sided_mm={int(allow_two_sided_mm)}"
        )
