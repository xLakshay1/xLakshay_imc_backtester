...
class Trader:
    def __init__(self):
        ...
        self.conversions = 0
        self.traderData = "MAINBH_OSMIUM_LADDER_ALPHA_MM_V1"
        ...
        self.osmium_ladder_gap_weight = 0.30
        self.osmium_ladder_gap_clip = 4.0
        self.osmium_alpha_clip = 4.0
    ...
    def get_osmium_alpha_features(self, state: TradingState, product: str):
        ...
        ask_ladder_gap = 0.0
        if len(bid_prices) >= 2:
            bid_ladder_gap = float(bid_prices[0] - bid_prices[1])
        if len(ask_prices) >= 2:
            ask_ladder_gap = float(ask_prices[1] - ask_prices[0])
        book_gap_asym = ask_ladder_gap - bid_ladder_gap
        ...
        return {
            ...
            "bid_ladder_gap": bid_ladder_gap,
            "ask_ladder_gap": ask_ladder_gap,
            "book_gap_asym": book_gap_asym,
        }
    ...
    def trade_osmium(self, state: TradingState):
        ...
        ladder_signal = self.clamp(
            features["book_gap_asym"],
            -self.osmium_ladder_gap_clip,
            self.osmium_ladder_gap_clip,
        )
        alpha_adjustment = (
            ...
            + self.osmium_ladder_gap_weight * ladder_signal
        )
        ...
        logger.print(
            f"OSMIUM_LADDER_ALPHA mid={mid:.2f} ema={self.osmium_fair_value:.2f} "
            f"micro_sig={micro_signal:.2f} depth_sig={depth_signal:.2f} "
            f"imb={top_imbalance:.2f} move={recent_move:.2f} "
            f"bid_gap={features['bid_ladder_gap']:.2f} ask_gap={features['ask_ladder_gap']:.2f} "
            f"gap_asym={features['book_gap_asym']:.2f} ladder_sig={ladder_signal:.2f} "
