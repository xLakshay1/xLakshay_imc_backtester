...
class Trader:
    def __init__(self):
        ...
        self.conversions = 0
        self.traderData = "MAINBH_OSMIUM_RET5_PRESSURE_PLUS_PEPPER_RECYCLE_V1"
        ...
        # Pepper: stay structurally long, but recycle a tiny slice when stretched.
        self.pepper_limit = 80
        self.pepper_clip_size = 80
        self.pepper_recycle_clip_size = 5
        self.pepper_core_position = 75
        self.pepper_mid_history: list[float] = []
        self.pepper_z_window = 60
        self.pepper_z_min_points = 30
        self.pepper_sell_z = 2.0
        self.pepper_strong_sell_z = 3.0
        self.pepper_rebuy_z = 1.2
    ...
    def update_pepper_z_score(self, mid: float) -> float:
        if len(self.pepper_mid_history) >= self.pepper_z_min_points:
            recent = self.pepper_mid_history[-self.pepper_z_window:]
            mean = sum(recent) / len(recent)
            variance = sum((value - mean) ** 2 for value in recent) / len(recent)
            std = math.sqrt(variance)
            z_score = (mid - mean) / std if std > 1e-9 else 0.0
        else:
            z_score = 0.0

        self.pepper_mid_history.append(mid)
        if len(self.pepper_mid_history) > self.pepper_z_window:
            self.pepper_mid_history = self.pepper_mid_history[-self.pepper_z_window:]
        return z_score
    ...
    def trade_pepper(self, state: TradingState):
        ...
        position = self.get_product_pos(state, product)
        mid = (best_bid + best_ask) / 2.0
        z_score = self.update_pepper_z_score(mid)
        logger.print(
            f"PEPPER_RECYCLE t={state.timestamp} bid={best_bid} ask={best_ask} "
            f"mid={mid:.2f} z60={z_score:.2f} pos={position}"
        )

        remaining_buy = max(0, self.pepper_limit - position - self.pepper_buy_orders)
        if len(self.pepper_mid_history) < self.pepper_z_min_points:
        ...
        if position < self.pepper_core_position:
            size = min(remaining_buy, self.pepper_core_position - position, self.pepper_clip_size)
            if size > 0:
                self.pepper_buy_orders += size
                self.send_buy_order(product, best_ask, size, f"PEPPER CORE BUY {size} @ {best_ask}")
            return

        if position < self.pepper_limit and z_score <= self.pepper_rebuy_z:
            size = min(remaining_buy, self.pepper_recycle_clip_size)
            if size > 0:
                self.pepper_buy_orders += size
                self.send_buy_order(product, best_ask, size, f"PEPPER REBUY {size} @ {best_ask} z={z_score:.2f}")
            return

        target_position = self.pepper_limit
        if z_score >= self.pepper_strong_sell_z:
            target_position = self.pepper_core_position
        elif z_score >= self.pepper_sell_z:
            target_position = self.pepper_limit - self.pepper_recycle_clip_size

        remaining_sell = max(0, position + self.pepper_limit - self.pepper_sell_orders)
        recycle_sell = max(0, position - target_position)
        size = min(remaining_sell, recycle_sell, self.pepper_recycle_clip_size)
        if size > 0:
            self.pepper_sell_orders += size
            self.send_sell_order(product, best_ask, -size, f"PEPPER RECYCLE SELL {size} @ {best_ask} z={z_score:.2f}")
            return

        if position < self.pepper_limit:
            size = min(remaining_buy, self.pepper_recycle_clip_size)
            if size > 0 and z_score < self.pepper_sell_z:
                self.pepper_buy_orders += size
                self.send_buy_order(product, best_ask, size, f"PEPPER TOPUP BUY {size} @ {best_ask} z={z_score:.2f}")
                return

        size = min(remaining_buy, self.pepper_clip_size)
        if size > 0 and position < self.pepper_limit:
            self.pepper_buy_orders += size
            self.send_buy_order(product, best_ask, size, f"PEPPER BUY {size} @ {best_ask}")
