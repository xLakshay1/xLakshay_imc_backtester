...
class Trader:
    def __init__(self):
        ...
        self.conversions = 0
        self.traderData = "MAINBH_OSMIUM_RET5_PRESSURE_SIZE_ALPHA_MM_V1"
        ...
        self.osmium_size_alpha_threshold = 0.75
        self.osmium_size_alpha_boost = 0.55
        self.osmium_size_alpha_cut = 0.45
    ...
    def trade_osmium(self, state: TradingState):
        ...
        alpha_abs = abs(alpha_adjustment)
        if alpha_abs > self.osmium_size_alpha_threshold:
            confidence = (alpha_abs - self.osmium_size_alpha_threshold) / (
                self.osmium_alpha_clip - self.osmium_size_alpha_threshold
            )
            confidence = self.clamp(confidence, 0.0, 1.0)
            boost = 1.0 + self.osmium_size_alpha_boost * confidence
            cut = 1.0 - self.osmium_size_alpha_cut * confidence

            if alpha_adjustment > 0:
                buy_quote_size = max(1, int(round(buy_quote_size * boost)))
                sell_quote_size = max(1, int(round(sell_quote_size * cut)))
            else:
                sell_quote_size = max(1, int(round(sell_quote_size * boost)))
                buy_quote_size = max(1, int(round(buy_quote_size * cut)))
