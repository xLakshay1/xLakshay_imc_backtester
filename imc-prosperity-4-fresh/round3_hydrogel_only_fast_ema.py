...
class Trader:
    def __init__(self):
        ...
        self.initial_ema = 9991.0
        self.ema_alpha = 0.12
        self.ret_weight = 0.18
        self.total_imb_weight = 1.10
        self.l1_imb_weight = 0.80
        self.inventory_skew = 0.90
        ...
        self.conversions = 0
        self.traderData = "R3_HYDROGEL_ONLY_FAST_EMA_V1"

    def parse_memory(self, trader_data: str) -> Dict[str, Any]:
        memory = {
            "ema": self.initial_ema,
            "prev_mid": None,
        }
        if not trader_data:
            return memory
        try:
            ...
            return memory
        memory["ema"] = float(raw.get("ema", self.initial_ema))
    ...
    def trade_hydrogel(self, state: TradingState, memory: Dict[str, Any]) -> None:
        ...
        ema = self.ema_alpha * mid + (1.0 - self.ema_alpha) * previous_ema
        memory["ema"] = ema
        ...
        take_threshold = max(2.0, 0.40 * float(signal["spread"]) / max(0.85, regime_scale))
        passive_offset = max(2.0, 0.35 * float(signal["spread"]) / max(0.85, regime_scale))
        ...
        if best_ask <= fair - take_threshold:
            self.place_buy(
                ...
                f"HYDRO TAKE BUY @ {best_ask} fair={fair:.2f} ema={ema:.2f}",
            )
        if best_bid >= fair + take_threshold:
            self.place_sell(
                ...
                f"HYDRO TAKE SELL @ {best_bid} fair={fair:.2f} ema={ema:.2f}",
            )
        ...
        self.place_buy(
            ...
            buy_price,
            quote_size,
            f"HYDRO MM BUY {quote_size} @ {buy_price} fair={fair:.2f} ema={ema:.2f}",
        )
        self.place_sell(
            state,
            sell_price,
            quote_size,
            f"HYDRO MM SELL {quote_size} @ {sell_price} fair={fair:.2f} ema={ema:.2f}",
        )

        logger.print(
            f"mid={mid:.2f} ema={ema:.2f} ret={recent_move:.2f} "
            f"imb={float(signal['imbalance']):.3f} l1={float(signal['l1_imbalance']):.3f} "
            f"pos={position}"
        )
