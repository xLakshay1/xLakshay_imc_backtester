...
class Trader:
    def __init__(self):
        ...
        self.conversions = 0
        self.traderData = "OSMIUM_SAFE_PLUS_PEPPER_BUY_THEN_SELL_V1"
        ...
        # Pepper trend strategy params
        self.pepper_limit = 80
        self.pepper_clip_size = 80
        self.pepper_exit_start = 970_000
    ...
    def trade_pepper(self, state: TradingState):
        ...
        logger.print(
            f"PEPPER_BUY_SELL bid={best_bid} ask={best_ask} pos={position}"
        )

        if state.timestamp >= self.pepper_exit_start:
            ...
            self.send_sell_order(
                ...
                best_bid,
                -size,
                msg=f"PEPPER END SELL {size} @ {best_bid}",
            )
            return
        ...
        if size > 0:
            ...
            self.send_buy_order(
                ...
                best_ask,
                size,
                msg=f"PEPPER START BUY {size} @ {best_ask}",
            )
