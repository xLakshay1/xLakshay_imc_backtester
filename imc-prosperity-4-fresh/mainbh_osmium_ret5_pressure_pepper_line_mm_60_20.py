...
class Trader:
    def __init__(self):
        ...
        self.conversions = 0
        self.traderData = "MAINBH_OSMIUM_RET5_PRESSURE_PEPPER_LINE_MM_60_20_V1"
        ...
        self.osmium_micro_weight = 0.25
        self.osmium_depth_trend_weight = 0.25
        self.osmium_top_imbalance_weight = 1.00
        ...
        # Pepper: hold a 70-unit structural long, then use the final 10 units
        # as a larger mean-reversion sleeve around a live-detected linear fair line.
        self.pepper_limit = 80
        self.pepper_core_target = 60
        self.pepper_sleeve_target = 80
        self.pepper_core_clip_size = 60
        self.pepper_sleeve_clip_size = 5
        self.pepper_line_slope = 0.001
        self.pepper_line_intercept = None
        self.pepper_line_alpha = 0.02
        self.pepper_mm_edge = 1.0
        self.pepper_quote_edge = 2.0
    ...
    def get_optional_best_bid_ask(self, state: TradingState, product: str):
        order_depth = state.order_depths[product]
    ...
    def trade_pepper(self, state: TradingState):
        ...
        best_bid, best_ask = self.get_optional_best_bid_ask(state, product)
        ...
        if best_bid is not None and best_ask is not None:
            mid = (best_bid + best_ask) / 2.0
            observed_intercept = mid - self.pepper_line_slope * state.timestamp
            if self.pepper_line_intercept is None:
                self.pepper_line_intercept = observed_intercept
            else:
                self.pepper_line_intercept = (
                    self.pepper_line_alpha * observed_intercept
                    + (1 - self.pepper_line_alpha) * self.pepper_line_intercept
                )
        elif self.pepper_line_intercept is None:
            # Need at least one two-sided tick before the residual sleeve has
            # a trustworthy fair line. The core long can still use ask-only rows.
            mid = None
        else:
            mid = None

        fair_value = None
        if self.pepper_line_intercept is not None:
            fair_value = self.pepper_line_intercept + self.pepper_line_slope * state.timestamp

        logger.print(
            f"PEPPER_LINE_MM t={state.timestamp} bid={best_bid} ask={best_ask} "
            f"pos={position} fair={fair_value if fair_value is not None else 'NA'}"
        )

        if best_ask is None:
            return

        # First build and keep the structural long. This is the high-conviction
        # drift trade, so do it aggressively and do not let the sleeve logic delay it.
        core_remaining = self.pepper_core_target - position - self.pepper_buy_orders
        size = min(core_remaining, self.pepper_core_clip_size)
        if size > 0:
            self.pepper_buy_orders += size
            self.send_buy_order(product, best_ask, size, f"PEPPER CORE BUY {size} @ {best_ask}")
            return

        if fair_value is None:
            return

        residual_bid = best_bid - fair_value if best_bid is not None else None
        residual_ask = best_ask - fair_value

        # Taker sleeve: buy only the extra 20 units when detrended price is cheap,
        # sell only those extra units when detrended price is rich.
        sleeve_buy_room = self.pepper_sleeve_target - position - self.pepper_buy_orders
        if residual_ask <= -self.pepper_mm_edge and sleeve_buy_room > 0:
            size = min(sleeve_buy_room, self.pepper_sleeve_clip_size)
            self.pepper_buy_orders += size
            self.send_buy_order(
                product,
                best_ask,
                size,
                f"PEPPER SLEEVE TAKE BUY {size} @ {best_ask} resid={residual_ask:.2f}",
            )
            return

        sleeve_sell_room = position - self.pepper_core_target - self.pepper_sell_orders
        if best_bid is not None and residual_bid is not None and residual_bid >= self.pepper_mm_edge and sleeve_sell_room > 0:
            size = min(sleeve_sell_room, self.pepper_sleeve_clip_size)
            self.pepper_sell_orders += size
            self.send_sell_order(
                product,
                best_bid,
                -size,
                f"PEPPER SLEEVE TAKE SELL {size} @ {best_bid} resid={residual_bid:.2f}",
            )
            return

        # Passive sleeve quotes around the fair line. The buy quote can build
        # extra inventory up to 80; the sell quote only unloads inventory above 70.
        if sleeve_buy_room > 0:
            buy_price = min(best_ask - 1, math.floor(fair_value - self.pepper_quote_edge))
            size = min(sleeve_buy_room, self.pepper_sleeve_clip_size)
            if size > 0:
                self.pepper_buy_orders += size
                self.send_buy_order(product, buy_price, size, f"PEPPER SLEEVE MM BUY {size} @ {buy_price}")

        if best_bid is not None and sleeve_sell_room > 0:
            sell_price = max(best_bid + 1, math.ceil(fair_value + self.pepper_quote_edge))
            size = min(sleeve_sell_room, self.pepper_sleeve_clip_size)
            if size > 0:
                self.pepper_sell_orders += size
                self.send_sell_order(product, sell_price, -size, f"PEPPER SLEEVE MM SELL {size} @ {sell_price}")
