...
class Trader:
    def __init__(self):
        ...
        self.conversions = 0
        self.traderData = "R3_VELVET_ONLY_EMA_V1"

    def parse_memory(self, trader_data: str) -> Dict[str, Any]:
        memory = {"ema": None, "prev_mid": None}
        if not trader_data:
            return memory
        try:
    ...
    def trade_velvet(self, state: TradingState, memory: Dict[str, Any]) -> None:
        ...
        ema = mid if prev_ema is None else self.ema_alpha * mid + (1.0 - self.ema_alpha) * float(prev_ema)
        ...
        if best_ask <= fair - take_threshold:
            self.place_buy(
                ...
                f"VELVET TAKE BUY @ {best_ask} fair={fair:.2f} ema={ema:.2f}",
            )
        if best_bid >= fair + take_threshold:
            self.place_sell(
                ...
                f"VELVET TAKE SELL @ {best_bid} fair={fair:.2f} ema={ema:.2f}",
            )
        ...
        self.place_buy(
            ...
            buy_price,
            quote_size,
            f"VELVET MM BUY {quote_size} @ {buy_price} fair={fair:.2f} ema={ema:.2f}",
        )
        self.place_sell(
            state,
            sell_price,
            quote_size,
            f"VELVET MM SELL {quote_size} @ {sell_price} fair={fair:.2f} ema={ema:.2f}",
        )

        logger.print(
            f"mid={mid:.2f} ema={ema:.2f} ret={recent_move:.2f} imb={float(signal['imbalance']):.3f} "
