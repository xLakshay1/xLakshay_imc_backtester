...
class Trader:
    def __init__(self):
        ...
        self.conversions = 0
        self.traderData = "CAND_NOMD_TOP05_HARDENED"
    ...
    def trim_orders_to_limits(
        ...
        for product, product_orders in orders.items():
            ...
            for order in product_orders:
                ...
                if quantity > 0:
                    original_buy += quantity
                    kept_quantity = min(quantity, buy_room)
                    if kept_quantity > 0:
                        trimmed_orders.append(Order(order.symbol, int(order.price), kept_quantity))
                        buy_room -= kept_quantity
                        kept_buy += kept_quantity
                elif quantity < 0:
                    sell_quantity = -quantity
                    original_sell += sell_quantity
                    kept_quantity = min(sell_quantity, sell_room)
                    if kept_quantity > 0:
                        trimmed_orders.append(Order(order.symbol, int(order.price), -kept_quantity))
                        sell_room -= kept_quantity
                        kept_sell += kept_quantity

            if original_buy != kept_buy or original_sell != kept_sell:
                logger.print(
                    f"SAFETY_TRIM product={product} "
                    f"pos={position} limit={limit} "
                    f"buy={kept_buy}/{original_buy} sell={kept_sell}/{original_sell}"
                )

            safe_orders[product] = trimmed_orders
    ...
    def run(self, state: TradingState):
        ...
        self.orders = self.trim_orders_to_limits(state, self.orders)
