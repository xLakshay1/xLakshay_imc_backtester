...
class Trader:
    def __init__(self):
        ...
        self.conversions = 0
        self.trade_alpha_weight = 0.10
        self.traderData = "R3_VELVET_ONLY_PREVDAY_MEAN_TAKER_V1"
    ...
    def trade_flow_signal(self, state: TradingState, signal: Dict[str, Any]) -> Dict[str, float]:
        ...
        for trade in state.market_trades.get(self.product, []):
            ...
            if price >= best_ask:
                ...
                side = "buy" if abs(price - best_ask) < abs(price - best_bid) else "sell"

            if side == "buy" and qty >= 6:
                buy_large += qty
            elif side == "sell" and qty < 6:
                sell_small += qty
            elif side == "sell" and qty >= 6:
                sell_large += qty

        raw_score = buy_large - 0.70 * sell_small - 0.85 * sell_large
        score = max(-16.0, min(16.0, raw_score))
        return {
            "buy_large": buy_large,
    ...
    def trade_velvet(self, state: TradingState, memory: Dict[str, Any]) -> None:
        ...
        if reversal_signal > 0 and best_ask <= math.floor(fair):
            self.place_buy(
                ...
                best_ask,
                reversal_room,
                f"VELVET REVERSAL BUY @ {best_ask} fair={fair:.2f} mids={memory['recent_mids']}",
            )
            ...
            self.place_sell(
                ...
                best_bid,
                reversal_room,
                f"VELVET REVERSAL SELL @ {best_bid} fair={fair:.2f} mids={memory['recent_mids']}",
            )

        logger.print(
            f"mid={mid:.2f} anchor={anchor:.2f} day_count={int(memory['day_count'])} "
            f"ret={recent_move:.2f} imb={float(signal['imbalance']):.3f} "
            f"l1={float(signal['l1_imbalance']):.3f} tf={float(trade_flow['score']):.1f} rev={reversal_signal} "
            f"bl={float(trade_flow['buy_large']):.0f} ss={float(trade_flow['sell_small']):.0f} "
            f"sl={float(trade_flow['sell_large']):.0f} pos={position}"
        )
