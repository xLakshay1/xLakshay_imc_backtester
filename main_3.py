...
class Trader:
    def __init__(self):
        ...
        # Hyperparameters
        self.tomatoes_take_threshold = 0.7
    ...
    def get_microprice(self, state: TradingState, product: str):
        order_depth = state.order_depths[product]
        if len(order_depth.buy_orders) == 0 or len(order_depth.sell_orders) == 0:
            return None
        ...
        return (best_ask * best_bid_volume + best_bid * best_ask_volume) / denom
    ...
    def trade_tomatoes(self, state: TradingState):
        ...
        position = self.get_product_pos(state, product)
        fair_value = self.get_microprice(state, product)
        if fair_value is None:
            return

        logger.print(
            f"TOMATOES bid={best_bid} ask={best_ask} microprice={fair_value:.2f} pos={position}"
        )
        ...
        self.search_buys(
            state,
            product,
            fair_value - self.tomatoes_take_threshold,
            depth=3,
        )
        self.search_sells(
            state,
