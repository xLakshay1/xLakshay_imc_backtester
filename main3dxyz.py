...
class Trader:
    def __init__(self):
        ...
        self.conversions = 0
        self.traderData = "EMA_MICROSTRUCTURE_V1"
    ...
    def trade_tomatoes(self, state: TradingState):
        ...
        # Short-horizon microstructure tilt
        ...
        # Adaptive quote offset from spread
        if spread >= 4:
            adaptive_quote_offset = 1.5
        else:
            adaptive_quote_offset = 2.5

        # Adaptive size from confidence
        quote_size = int(self.tomatoes_max_quote_size * (0.6 + 0.7 * confidence))
        quote_size = max(6, min(20, quote_size))

        if abs(position) > 50:
            quote_size = min(8, quote_size)
        elif abs(position) > 30:
            quote_size = min(12, quote_size)
        ...
        buy_offset = adaptive_quote_offset + max(0, inventory_skew)
        sell_offset = adaptive_quote_offset + max(0, -inventory_skew)
