...
class Trader:
    def __init__(self):
        ...
        self.tomatoes_last_mid = None
        self.tomatoes_volatility = None

        # Ultra-safe params
        self.tomatoes_alpha = 0.08
        self.tomatoes_vol_alpha = 0.03
        self.tomatoes_base_spread = 3.5
        self.tomatoes_take_size = 5
        self.tomatoes_quote_size = 6
    ...
    def trade_tomatoes(self, state: TradingState):
        ...
        # Very smooth volatility estimate
        if self.tomatoes_last_mid is None:
            price_change = 0.0
        else:
            price_change = abs(mid - self.tomatoes_last_mid)

        if self.tomatoes_volatility is None:
            self.tomatoes_volatility = price_change
        else:
            self.tomatoes_volatility = (
                self.tomatoes_vol_alpha * price_change
                + (1 - self.tomatoes_vol_alpha) * self.tomatoes_volatility
            )

        self.tomatoes_last_mid = mid

        vol = self.tomatoes_volatility
        spread = self.tomatoes_base_spread + 2 * vol

        # Only take very strong edges
        if best_ask < fair_value - spread:
            size = min(self.tomatoes_take_size, self.limits[product] - position)
            if size > 0:
                self.send_buy_order(
                    product,
                    best_ask,
                    size,
                    msg=f"{product} TAKE BUY {size} @ {best_ask}",
                )
        ...
        if best_bid > fair_value + spread:
            ...
            if size > 0:
                self.send_sell_order(
                    ...
                    best_bid,
                    -size,
                    msg=f"{product} TAKE SELL {size} @ {best_bid}",
                )

        # Strong inventory penalty
        inventory_skew = position / 10.0

        buy_price = min(
            best_bid + 1,
            math.floor(fair_value - spread - max(0, inventory_skew))
        )
        sell_price = max(
            best_ask - 1,
            math.ceil(fair_value + spread + max(0, -inventory_skew))
        )
        ...
        # Very small quote sizes
        quote_size = self.tomatoes_quote_size
        if abs(position) > 20:
            quote_size = 3
        if abs(position) > 40:
            quote_size = 1
        ...
        if max_buy > 0:
            self.send_buy_order(
                ...
                buy_price,
                max_buy,
                msg=f"{product} SAFE MM BUY {max_buy} @ {buy_price}",
            )
        ...
        if max_sell > 0:
            self.send_sell_order(
                ...
                sell_price,
                -max_sell,
                msg=f"{product} SAFE MM SELL {max_sell} @ {sell_price}",
            )

        logger.print(
            f"{product} SAFE mid={mid:.2f} fair={fair_value:.2f} vol={vol:.2f} pos={position}"
        )
